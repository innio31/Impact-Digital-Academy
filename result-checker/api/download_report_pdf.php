...
if (!$use_pdf) {
    die("PDF library not found. Please install TCPDF in includes/tcpdf/");
}
...
try {
    ...
    if (!empty($scores)) {
        $firstScore = $scores[0];
        if (isset($firstScore['components'])) {
            $score_types = array_keys($firstScore['components']);
        } elseif (isset($firstScore['score_data'])) {
            $score_types = array_keys($firstScore['score_data']);
            // Remove non-score keys
            $score_types = array_filter($score_types, function ($key) {
                return !in_array($key, ['subject_name', 'subject_id', 'raw_values']);
            });
        }
    }
    ...
    $html = generatePDFHTML($student, $result, $scores, $affective_traits, $psychomotor_skills, $score_types, $total_scored, $total_max, $percentage, $age);
...
function generatePDFHTML($student, $result, $scores, $affective_traits, $psychomotor_skills, $score_types, $total_scored, $total_max, $percentage, $age)
{
    // Helper functions
    function getGradePDF($percentage)
    {
    ...
    function getRemarkPDF($percentage)
    {
    ...
    $overall_remark = getRemarkPDF($percentage);
    ...
    // Fill empty rows to reach 15 subjects (for consistent layout)
    ...
    if ($affective_traits) {
        foreach ($affective_items as $key => $label) {
            ...
            $affective_html .= '<tr><td style="border:1px solid #000; padding:4px;">' . $label . '</td><td style="border:1px solid #000; padding:4px; text-align:center;"><strong>' . htmlspecialchars($value) . '</strong></td></tr>';
        }
    }
    ...
    if ($psychomotor_skills) {
        foreach ($psychomotor_items as $key => $label) {
            $value = $psychomotor_skills[$key] ?? '-';
            $psychomotor_html .= '<tr><td style="border:1px solid #000; padding:4px;">' . $label . '</td><td style="border:1px solid #000; padding:4px; text-align:center;"><strong>' . htmlspecialchars($value) . '</strong></td></tr>';
        }
    }
